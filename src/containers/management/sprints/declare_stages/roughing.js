...
class DeclareStagesRoughingSprint extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...buttons,
        ...
        //this.handleAddSyncStep = this.handleAddSyncStep.bind(this);
    ...
    componentWillReceiveProps(nextProps) {
        const { listAllStageRoughing } = this.props;
        if (nextProps.listAllStageRoughing !== listAllStageRoughing) {
            this.setState({
                newLine: [
                    <tr key={nextProps.listAllStageRoughing.length}>
                        <td>
                            <Input typeInput="checkbox" />
                            {nextProps.listAllStageRoughing.length + 1}.
                            <Input
                                typeInput="text"
        ...
        this.setState({
            listAllSteps: nextProps.listAllStageRoughing
        })
    ...
    handleAddStep() {
        ...
        const { listAllStageRoughing } = this.props;
        newLine.push(
            <tr key={listAllStageRoughing.length + newLine.length + 1}>
                <td>
                    <Input typeInput="checkbox" />
                    {listAllStageRoughing.length + newLine.length + 1}.
                    <Input
                        typeInput="text"
                        placeholderInput="Input text"
                        onKeyDownInput={this.postStage}
    ...
    async postStage(event) {
        if (
            ...
            await this.props.postRoughingStage({
                dataPost
            });
        }
    }
...
const mapDispatchToProps = dispatch => ({
    ...
    postRoughingStage: data => dispatch(postRoughingStage(data))
});
